...
async function main() {
   ...
   const PIZZA_CHEF_ADDR = "0xB523A28700E84E6D602C65A353bd3f313f74D204";
   const rewardTokenTicker = "PIZZA";
   const PIZZA_CHEF = new ethers.Contract(PIZZA_CHEF_ADDR, PIZZA_CHEF_ABI, App.provider);

   const startBlock = await PIZZA_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await PIZZA_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await PIZZA_CHEF.PizzaPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, PIZZA_CHEF, PIZZA_CHEF_ADDR, PIZZA_CHEF_ABI, rewardTokenTicker,
        "pizza", null, rewardsPerWeek, "pendingPizza");
