...
async function main() {
   ...
   const XENO_CHEF_ADDR = "0x74c50d283046404A87Eaa7705f7227f09e5a91Dc";
   const rewardTokenTicker = "XENO";
   const XENO_CHEF = new ethers.Contract(XENO_CHEF_ADDR, XENO_CHEF_ABI, App.provider);

   const startBlock = await XENO_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await XENO_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await XENO_CHEF.xenoPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, XENO_CHEF, XENO_CHEF_ADDR, XENO_CHEF_ABI, rewardTokenTicker,
        "xeno", null, rewardsPerWeek, "pendingXeno");
