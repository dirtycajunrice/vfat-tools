...
async function main() {
   ...
   const ALADDIN_CHEF_ADDR = "0x3Acf084BF1631E4F2B282F800Ff07E9e760957fD";
   const rewardTokenTicker = "ALADDIN";
   const ALADDIN_CHEF = new ethers.Contract(ALADDIN_CHEF_ADDR, ALADDIN_CHEF_ABI, App.provider);

   const startBlock = await ALADDIN_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await ALADDIN_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await ALADDIN_CHEF.aladdinPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, ALADDIN_CHEF, ALADDIN_CHEF_ADDR, ALADDIN_CHEF_ABI, rewardTokenTicker,
        "aladdin", null, rewardsPerWeek, "pendingAladdin");
