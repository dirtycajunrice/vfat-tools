...
async function main() {
   ...
   const SVIKI_CHEF_ADDR = "0xF9Ba68e0Fa6C217b7849a18307358Ee27a71b99a";
   const rewardTokenTicker = "SVIKI";
   const SVIKI_CHEF = new ethers.Contract(SVIKI_CHEF_ADDR, SVIKI_CHEF_ABI, App.provider);

   const startBlock = await SVIKI_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await SVIKI_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await SVIKI_CHEF.svikiPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, SVIKI_CHEF, SVIKI_CHEF_ADDR, SVIKI_CHEF_ABI, rewardTokenTicker,
        "sviki", null, rewardsPerWeek, "pendingSviki");
