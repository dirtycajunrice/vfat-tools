...
async function main() {
   ...
   const KLING_CHEF_ADDR = "0x4de6c2de6b9eBD974738686C9be7a31597146Ac6";

   const rewardTokenTicker = "KLING";
   const KLING_CHEF = new ethers.Contract(KLING_CHEF_ADDR, KLING_CHEF_ABI, App.provider);

   let rewardsPerWeek = 0
   const startBlock = await KLING_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   if(currentBlock < startBlock){
     _print(`Rewards start at block ${startBlock}\n`);
   }else{
    rewardsPerWeek = await KLING_CHEF.lqdrPerBlock() /1e18 * 604800;
   }
