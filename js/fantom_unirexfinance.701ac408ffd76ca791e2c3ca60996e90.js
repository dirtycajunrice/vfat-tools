...
$(function() {
    ...
    async function main() {
       ...
       const UNIREX_CHEF_ADDR = "0x75417A5647B88dc50F341cbEd5dB9aD6C3027Ed5";
    
       const rewardTokenTicker = "UNIREX";
       const UNIREX_CHEF = new ethers.Contract(UNIREX_CHEF_ADDR, UNIREX_CHEF_ABI, App.provider);
    
       let rewardsPerWeek = 0
       const startBlock = await UNIREX_CHEF.startBlock();
       const currentBlock = await App.provider.getBlockNumber();
    
       const multiplier = await UNIREX_CHEF.getMultiplier(currentBlock, currentBlock+1);
    
       if(currentBlock < startBlock){
        _print(`Rewards start at block <a href="https://ftmscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`);
       }else{
        rewardsPerWeek = await UNIREX_CHEF.UnirexPerBlock() /1e18 * multiplier * 604800;
       }
