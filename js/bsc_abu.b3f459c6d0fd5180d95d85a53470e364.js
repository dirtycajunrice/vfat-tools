...
async function main() {
   ...
   const ABU_CHEF_ADDR = "0x312bF8dC3BF5CF58694f30958667cBCbf35B35b8";
   const rewardTokenTicker = "ABU";
   const ABU_CHEF = new ethers.Contract(ABU_CHEF_ADDR, ABU_CHEF_ABI, App.provider);

   const startBlock = await ABU_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await ABU_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await ABU_CHEF.abuPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, ABU_CHEF, ABU_CHEF_ADDR, ABU_CHEF_ABI, rewardTokenTicker,
        "abu", null, rewardsPerWeek, "pendingAbu");
