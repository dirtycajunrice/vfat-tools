...
async function main() {
   ...
   const BEAN_CHEF_ADDR = "0xe8cB74905b47B579dd4fCABc7682f1e0d2f98308";
   const rewardTokenTicker = "BEAN";
   const BEAN_CHEF = new ethers.Contract(BEAN_CHEF_ADDR, BEAN_CHEF_ABI, App.provider);

   const startBlock = await BEAN_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await BEAN_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await BEAN_CHEF.BeanPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, BEAN_CHEF, BEAN_CHEF_ADDR, BEAN_CHEF_ABI, rewardTokenTicker,
        "bean", null, rewardsPerWeek, "pendingBean");
