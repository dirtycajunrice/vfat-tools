...
async function main() {
   ...
   const KIMCHI_MC_ADDR = "0x544e849edaeB7177b223848405453828ef937CAA";
   const rewardTokenTicker = "KIMCHI";
   const KIMCHI_MC = new ethers.Contract(KIMCHI_MC_ADDR, KIMCHI_CHEF_ABI, App.provider);

   const startBlock = await KIMCHI_MC.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await KIMCHI_MC.getMultiplier(currentBlock, currentBlock + 1);
   ...
   rewardsPerWeek = await KIMCHI_MC.kimchiPerBlock() / 1e18 * 604800 * multiplier / 2.1;

    const tokens = {};
    const prices = await getMaticPrices();

    await loadMaticChefContract(App, tokens, prices, KIMCHI_MC, KIMCHI_MC_ADDR, KIMCHI_CHEF_ABI, rewardTokenTicker,
        "kimchi", null, rewardsPerWeek, "pendingKimchi");
