...
async function main() {
   ...
   const rewardsPerWeek0 = await SDO_CHEF.tokensPerBlock() /1e18
        * 604800 / 3;
   const rewardsPerWeek1 = await SSO_CHEF.tokensPerBlock() /1e18
        * 604800 / 3;
    ...
    await loadSupraDollarChefContract(App, tokens, prices, SDO_CHEF, SDO_CHEF_ADDR, SDO_CHEF_ABI, rewardTokenTicker0,
      "SDO", null, rewardsPerWeek0, "pendingReward", 12);
    await loadSupraDollarChefContract(App, tokens, prices, SSO_CHEF, SSO_CHEF_ADDR, SSO_CHEF_ABI, rewardTokenTicker1,
        "sso", null, rewardsPerWeek1, "pendingReward", 7);

    let br = await loadBoardroom(App, prices, SDO_BOARDROOM_ADDR, SDO_ORACLE_ADDR, lptAddress, rewardTokenAddress, stakeTicker,
