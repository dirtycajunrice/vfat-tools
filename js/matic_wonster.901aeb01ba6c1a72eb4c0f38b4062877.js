...
async function main() {
   ...
   const WONSTER_CHEF_ADDR = "0xd3Cf795b0B09d8b07084c4454F8f797a6BF7B8dC";
   const rewardTokenTicker = "WONSTER";
   const WONSTER_CHEF = new ethers.Contract(WONSTER_CHEF_ADDR, WONSTER_CHEF_ABI, App.provider);

   const startBlock = await WONSTER_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await WONSTER_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await WONSTER_CHEF.WONSTERPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, WONSTER_CHEF, WONSTER_CHEF_ADDR, WONSTER_CHEF_ABI, rewardTokenTicker,
        "WONSTER", null, rewardsPerWeek, "pendingWONSTER");
