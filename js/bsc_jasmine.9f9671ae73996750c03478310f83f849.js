...
async function main() {
   ...
   const JASMINE_CHEF_ADDR = "0x9FF66EeBe0C8deD556C240A7134a2e680Cd7ab2b";
   const rewardTokenTicker = "JASMINE";
   const JASMINE_CHEF = new ethers.Contract(JASMINE_CHEF_ADDR, JASMINE_CHEF_ABI, App.provider);

   const startBlock = await JASMINE_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await JASMINE_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await JASMINE_CHEF.jasminePerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, JASMINE_CHEF, JASMINE_CHEF_ADDR, JASMINE_CHEF_ABI, rewardTokenTicker,
        "jasmine", null, rewardsPerWeek, "pendingJasmine");
