...
async function main() {
   ...
   const SMAN_CHEF_ADDR = "0xaF2cF6dbfc8217dE157EE8d1b6Ae86653A4d3F21";
   const rewardTokenTicker = "SMAN";
   const SMAN_CHEF = new ethers.Contract(SMAN_CHEF_ADDR, SMAN_CHEF_ABI, App.provider);

   const startBlock = await SMAN_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await SMAN_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await SMAN_CHEF.SuperManionPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, SMAN_CHEF, SMAN_CHEF_ADDR, SMAN_CHEF_ABI, rewardTokenTicker,
        "SMAN", null, rewardsPerWeek, "pendingSuperManion");
