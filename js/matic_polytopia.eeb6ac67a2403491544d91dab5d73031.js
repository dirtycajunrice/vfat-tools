...
async function main() {
   ...
   const UTOPIA_CHEF_ADDR = "0xd2Fb5e6DF8B468Ae57E7fefD942C44F275a56710";
   const rewardTokenTicker = "UTOPIA";
   const UTOPIA_CHEF = new ethers.Contract(UTOPIA_CHEF_ADDR, UTOPIA_CHEF_ABI, App.provider);

   const startBlock = await UTOPIA_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await UTOPIA_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await UTOPIA_CHEF.UTopiaPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, UTOPIA_CHEF, UTOPIA_CHEF_ADDR, UTOPIA_CHEF_ABI, rewardTokenTicker,
        "UTopia", null, rewardsPerWeek, "pendingUTopia");
