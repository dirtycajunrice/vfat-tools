...
async function main() {
   ...
   const HALVAR_CHEF_ADDR = "0xD864dad6991ABb582A096eeeD15AEE780F712bAA";
   const rewardTokenTicker = "HALVAR";
   const HALVAR_CHEF = new ethers.Contract(HALVAR_CHEF_ADDR, HALVAR_CHEF_ABI, App.provider);

   const startBlock = await HALVAR_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await HALVAR_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await HALVAR_CHEF.HalvarPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, HALVAR_CHEF, HALVAR_CHEF_ADDR, HALVAR_CHEF_ABI, rewardTokenTicker,
        "HALVAR", null, rewardsPerWeek, "pendingHalvar");
