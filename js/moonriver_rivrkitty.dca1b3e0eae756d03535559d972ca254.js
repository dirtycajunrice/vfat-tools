...
async function main() {
  ...
  const PAWS_CHEF_ADDR = '0x995ef3a5D14b66Ac5C7Fa1a967F8D9Cd727452bA'
  const PAWS_CHEF = new ethers.Contract(PAWS_CHEF_ADDR, PAWS_CHEF_ABI, App.provider)
  const rewardTokenTicker = 'PAWS'

  const blocksPerSeconds = await getAverageBlockTime(App)
  const rewardsPerWeek = (((await PAWS_CHEF.pawsPerBlock()) / 1e18) * 604800) / blocksPerSeconds

  const tokens = {}
  const prices = await getMoonriverPrices();
  
  let res = await $.ajax({
    url: 'https://api.coingecko.com/api/v3/simple/price?ids=rivrkitty&vs_currencies=usd',
    type: 'GET',
  })
