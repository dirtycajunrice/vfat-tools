...
async function main() {
   ...
   const startBlock = await iBG_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await iBG_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await iBG_CHEF.ibgPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, iBG_CHEF, iBG_CHEF_ADDR, iBG_CHEF_ABI, rewardTokenTicker,
        "ibg", null, rewardsPerWeek, "pendingIBG", [1]);
