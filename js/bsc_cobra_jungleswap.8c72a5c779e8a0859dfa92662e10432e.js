...
async function main() {
   ...
   const COBRA_CHEF_ADDR = "0x3dB9789A3846d8C3e04DF55CEAfeF4Dc7ef37608";
   const rewardTokenTicker = "COBRA";
   const COBRA_CHEF = new ethers.Contract(COBRA_CHEF_ADDR, COBRA_CHEF_ABI, App.provider);

   const startBlock = await COBRA_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await COBRA_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await COBRA_CHEF.cobraPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, COBRA_CHEF, COBRA_CHEF_ADDR, COBRA_CHEF_ABI, rewardTokenTicker,
        "cobra", null, rewardsPerWeek, "pendingCobra");
