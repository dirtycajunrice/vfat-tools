...
async function main() {
   ...
   const MOD_CHEF_ADDR = "0x2d111b3817dEF68ECce28b87F2286ECCB1a27674";
   const rewardTokenTicker = "MOD";
   const MOD_CHEF = new ethers.Contract(MOD_CHEF_ADDR, MOD_CHEF_ABI, App.provider);

   const startBlock = await MOD_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await MOD_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await MOD_CHEF.modPerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, MOD_CHEF, MOD_CHEF_ADDR, MOD_CHEF_ABI, rewardTokenTicker,
        "mod", null, rewardsPerWeek, "pendingMod");
