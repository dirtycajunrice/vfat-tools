...
async function main() {
   ...
   const SNORRE_CHEF_ADDR = "0x96B86e23C7425b36892fD07cc412851636D76BcB";
   const rewardTokenTicker = "SNORRE";
   const SNORRE_CHEF = new ethers.Contract(SNORRE_CHEF_ADDR, SNORRE_CHEF_ABI, App.provider);

   const startBlock = await SNORRE_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await SNORRE_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await SNORRE_CHEF.snorrePerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, SNORRE_CHEF, SNORRE_CHEF_ADDR, SNORRE_CHEF_ABI, rewardTokenTicker,
        "snorre", null, rewardsPerWeek, "pendingSnorre");
