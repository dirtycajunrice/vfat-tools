...
async function main() {
   ...
   const SUSHI_CHEF_ADDR = "0xbCbCE57fe370ee153EAafFa4BFFF1dE686dEE1Bd";
   const rewardTokenTicker = "SUSHI";
   const SUSHI_CHEF = new ethers.Contract(SUSHI_CHEF_ADDR, SUSHI_CHEF_ABI, App.provider);

   const startBlock = await SUSHI_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await SUSHI_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await SUSHI_CHEF.sushiPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, SUSHI_CHEF, SUSHI_CHEF_ADDR, SUSHI_CHEF_ABI, rewardTokenTicker,
