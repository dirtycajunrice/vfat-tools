...
async function main() {
   ...
   const BLOOD_CHEF_ADDR = "0xb0faE26e5C3282A5c919bBA32d168aAcD83B8194";
   const rewardTokenTicker = "BLOOD";
   const BLOOD_CHEF = new ethers.Contract(BLOOD_CHEF_ADDR, BLOOD_CHEF_ABI, App.provider);

   const startBlock = await BLOOD_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await BLOOD_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await BLOOD_CHEF.bloodPerBlock() /1e18
