...
async function main() {
   ...
   const FAXE_CHEF_ADDR = "0xd1AEb6be9c4a8b6F7E8AAf6d6395B210EacaD6C8";
   const rewardTokenTicker = "FAXE";
   const FAXE_CHEF = new ethers.Contract(FAXE_CHEF_ADDR, FAXE_CHEF_ABI, App.provider);

   const startBlock = await FAXE_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await FAXE_CHEF.getMultiplier(currentBlock, currentBlock+1);

   let rewardsPerWeek = 0
   if(currentBlock < startBlock){
    _print(`Rewards start at block <a href="https://bscscan.com/block/countdown/${startBlock}" target="_blank">${startBlock}</a>\n`)
   }else{
    rewardsPerWeek = await FAXE_CHEF.FaxePerBlock() /1e18
        * 604800 * multiplier / 3;
   }
    ...
    await loadBscChefContract(App, tokens, prices, FAXE_CHEF, FAXE_CHEF_ADDR, FAXE_CHEF_ABI, rewardTokenTicker,
        "FAXE", null, rewardsPerWeek, "pendingFaxe");
