...
async function main() {
   ...
   const BEE_CHEF_ADDR = "0x0895FC75e52e429cd32B175CD4DdECBf486B4e65";
   const rewardTokenTicker = "BEE";
   const BEE_CHEF = new ethers.Contract(BEE_CHEF_ADDR, BEE_CHEF_ABI, App.provider);

   const startBlock = await BEE_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await BEE_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await BEE_CHEF.beefarmPerBlock() /1e18
    ...
    await loadMaticChefContract(App, tokens, prices, BEE_CHEF, BEE_CHEF_ADDR, BEE_CHEF_ABI, rewardTokenTicker,
        "beefarm", null, rewardsPerWeek, "pendingBeeFarm", [15]);
